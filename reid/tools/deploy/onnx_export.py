...
if __name__ == '__main__':
    ...
    dynamic = {'images': {0: 'batch'}, 'output': {0: 'batch'}}  # shape(1,3,640,640)

    save_path = os.path.join(args.output, args.name+'.onnx')
    onnx_model = torch.onnx.export(model, inputs, save_path, input_names=['images'], output_names=['output'], dynamic_axes=dynamic)
    # onnx_model = export_onnx_model(model, inputs, input_names=['images'], output_names=['output'], dynamic_axes=dynamic)
