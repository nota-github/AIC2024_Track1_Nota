...
val_gt_files = Path('data/videos/val').glob('**/ground_truth.txt')
last_obj_id = 0
os.makedirs(f'data/reid_dataset/query', exist_ok=True)
os.makedirs(f'data/reid_dataset/bounding_box_test', exist_ok=True)

for gt_file in val_gt_files:
    print(gt_file)
    ...
    for label in tqdm(labels):
        ...
        img_path = f"data/frames/val/{scene}/camera_{formatted_cam_id}/{scene}camera_{formatted_cam_id}_{frame_id}.jpg"
    ...
    for obj_id in tqdm(obj_id_2_set):
        obj_set = obj_id_2_set[obj_id]
        random.shuffle(obj_set)
        sampled_query = obj_set[:5]
        sampled_gallery = obj_set[5:50]

        for obj in sampled_query:
            im_path, box, reid_im_name = obj
            x1, y1, x2, y2 = box
            reid_path = f"data/reid_dataset/query/{reid_im_name}"
            img = cv2.imread(im_path)
            cropped_img = img[y1:y2, x1:x2]
            cv2.imwrite(reid_path, cropped_img)
        
        for obj in sampled_gallery:
            im_path, box, reid_im_name = obj
            x1, y1, x2, y2 = box
            reid_path = f"data/reid_dataset/bounding_box_test/{reid_im_name}"
            img = cv2.imread(im_path)
            cropped_img = img[y1:y2, x1:x2]
            cv2.imwrite(reid_path, cropped_img)
