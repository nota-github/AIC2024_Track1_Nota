...
class Clustering:
    ...
    def update(self, trackers, cur_frame, scene):
        ...
        for a_tracker, b_tracker in tracker_pairs:
            ...
            for i, track in enumerate(a_tracker.tracked_stracks):
                ...
                if h/w < 1.5 or has_points is None or sum(has_points) <= 8 or sum(has_heads) <= 2 or track.num_kpts >= 18:
                    track.global_id = -1
                    continue
                # 추가
            ...
            # print('a_locations: ', a_locations)
            # print('b_locations: ', b_locations)
            ...
            # elif scene=='scene_041':
            #     dists[emb_dists > 0.30] = 1.0  # 몇으로 할 지 중요!!!
            #     dists[euc_dists > 1] = 1.0  # 몇으로 할 지 중요!!!!!! e.g. 0.15 -> 4, 5, etc
            # elif scene=='scene_042':
            else:
                # dists[emb_dists > 0.35] = 1.0  # 몇으로 할 지 중요!!!
                dists[emb_dists > 0.325] = 1.0  # 몇으로 할 지 중요!!!
                # print('\nclustering euc_disits: \n', euc_dists)
                # print('clustering emb disits: \n', emb_dists, '\n')
                dists[euc_dists > 1.0] = 1.0  # 몇으로 할 지 중요!!!!!! e.g. 0.15 -> 4, 5, etc

            # print('dists: ', dists)
            matches, u_afeats, u_bfeats = matching.linear_assignment(dists, thresh=0.999)  # 기존
            # matches, u_afeats, u_bfeats = matching.linear_assignment(dists, thresh=0.80)  # 추가 !!!
    ...
    def pose_check_all(self, pose_):
        ...
        #                 left_knee, right_knee, left_ankle, right_ankle]) > 0.3, np.array([shoulder, elbow, wrist, hip, knee, ankle]) > 0.3
    ...
    def update_using_mctracker(self, trackers, mc_tracker):
        ...
        for tracker in trackers:
            ...
            # tracks = [track for track in tracker.tracked_stracks if track.global_id != -1]  # clustering 할때는 high pose만 가지고 -> update_u_mctrack 때는 모두
            ...
            # print('update_using_mctracker: ', dists)
            matches, u_scts, u_mcts = matching.linear_assignment(dists, thresh=2)  # 몇으로 할 지 중요!!!!!!!!!! 0.25 -> 4, 5, etc
            # matches, u_scts, u_mcts = matching.linear_assignment(dists, thresh=20)  # 몇으로 할 지 중요!!!!!!!!!! 0.25 -> 4, 5, etc 
            for isct, imct in matches:
